...
async function main() {
  ...
  const [review, etfStableReview] = await Promise.all([
    generateWeeklyReview(),
    generateEtfStableWeeklyReview(),
  ]);
  const line = JSON.stringify({
    ranAt: new Date().toISOString(),
    id: review.id,
    etfStableWeekEnd: etfStableReview.weekEnd,
  });
  appendFileSync(LOG_PATH, `${line}\n`, 'utf-8');
  process.stdout.write(JSON.stringify({ review, etfStableReview }));
}
