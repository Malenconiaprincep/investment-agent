/** 与 packages/agent-core/src/mastra/config/model-providers.ts 保持同步 */
...
const allModelIds = new Set(
  MODEL_PROVIDERS.flatMap((p) => p.models.map((m) => m.id)),
);
...
export function isKnownModelId(modelId: string): boolean {
  return allModelIds.has(modelId.trim());
}

export function resolveModelIdFromKeys(
  keys: Record<string, { configured?: boolean; value?: string } | undefined>,
): string {
  const configured = keys[AI_MODEL_ENV]?.value?.trim();
