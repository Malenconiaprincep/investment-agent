...
// ../../packages/agent-core/src/data/reports/store.ts
...
function mapRow(row) {
  return {
    ...
    missingSections: JSON.parse(String(row.missing_sections)),
    missingKeywords: JSON.parse(String(row.missing_keywords)),
    elapsedMs: row.elapsed_ms === null || row.elapsed_ms === void 0 ? null : Number(row.elapsed_ms),
...
async function listResearchReports(options = {}) {
  const db = await getDb();
  const limit = options.limit ?? 50;
  const result = options.symbol ? await db.execute({
...
// ../../packages/agent-core/src/cli/reports-json.ts
async function main() {
  ...
  if (command === "list") {
    ...
    const reports = await listResearchReports({
      symbol: symbol && /^\d{6}$/.test(symbol) ? symbol : void 0,
      limit: 100
    });
